...
def parse_compound(cheq):
    """
    Parse the compounds and separate in elements
    cheq: str
    """
    cheq_l, cheq_r = cheq.replace(" ","").split('=')
    compound_left=cheq_l.split('+')
    compound_right=cheq_r.split('+')
...
def get_compd2(cheq):
    ...
    compound_left=cheq_l.split('+')
    compound_right=cheq_r.split('+')
    compound_left = [extent2(x) for x in compound_left]
    compound_right = [extent2(x) for x in compound_right]
    return compound_left,compound_right
...
def balancer(cheq, steps = False):
    ...
    # compd1,compd2 = parse_compound(cheq)
    #     print("提取反应物和生成物")
    #     print("    ",end = '')
    #     print(compd1,compd2)
    # compound_left,compound_right = get_compd2(cheq) 
    #     print("反应物和生成物进行变换")
    #     print("    ",end = '')
    #     print(compound_left,compound_right)
    # #得到所有参与的元素
    # elems = get_elem(compound_right)
    #     print("提取参与反应的元素")
    #     print("    ",end = '')
    #     print(elems)
    # #生成线性方程组的矩阵
    # F = count_elem(compound_left,compound_right,elems)
    #     print("生成线性方程组的系数矩阵")
    #     for f in F:
    #         print("    ",end = '')
    #         print(f)
    ...
    #     coe = ''
    #     if v[i+len(compound_left)] != 1:
    #         coe = v[i+len(compound_left)]
    #     cheq_balanced+=str(coe) + c + " + "
    # cheq_balanced = cheq_balanced[:-2]
    #     print("配平好的方程")
    #     print("    ",end = '')
...
if __name__ =='__main__':
    ...
    c6 = "H2+Ca(CN)2+NaAlF4+FeSO4+MgSiO3+KI+H3PO4+PbCrO4+BrCl+CF2Cl2+SO2=PbBr2+CrCl3+MgCO3+KAl(OH)4+Fe(SCN)3+PI3+Na2SiO3+CaF2+H2O"
    c4 = "KMnO4+H2SO4+FeSO4=Fe2(SO4)3+MnSO4+K2SO4+H2O"
    res = balancer(c6, True)
    print(res)
    """
    print(balancer("KMnO4+H2SO4+FeSO4=Fe2(SO4)3+MnSO4+K2SO4+H2O", False))
